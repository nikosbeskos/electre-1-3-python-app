...
class Store_data:   # Stores the data from the file
    ...
    # q =       np.array([20, 1, 1, 1])  # Indifference Threshold
    # p =       np.array([70, 1, 1, 1])  # Preference Threshold
...
def load_data(class_: Store_data, ismanual: bool, path=None):       # noqa loads data from file_path, cleans it, and dumps it in Store_data
    ...
    df = df.set_axis(labels=df.iloc[0], axis='columns', copy=False)
    ...
    if check != 0:
        ...
        print(f'There has to be something wrong with the file. filepath: {filepath}')   # noqa
    return x
